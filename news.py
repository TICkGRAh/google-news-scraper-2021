...
Results Gathered: %s
File Location: %s''' %
		(args["keyword"].strip(), len(all_data), os.path.join(os.getcwd(), output, keyword.replace("+", "-") + f'({loc}).csv')))
		return output_csv(all_data, keyword, loc)

	elif output == 'json':
		print('''Finished scraping for keyword: %s
Results Gathered: %s
File Location: %s''' %
		(args["keyword"].strip(), len(all_data), os.path.join(os.getcwd(), output, keyword.replace("+", "-") + f'({loc}).json')))
		return output_json(all_data, keyword, loc)
